...
**total 66 | done 66 | open 0 | blocked 0 | S0:1 S1:16 S2:22 S3:27**
...
| F-059 | S1 | process | DONE | `AUDIT/ledger.md, commit d165037` | A 'git checkout --' discarded an uncommitted product change, and the commit message described the change that had just been lost |
...
| F-054 | S3 | reporting | DONE | `FXNews.mq5 (historical symbol skip message)` | The chart symbol's M1 history is only built on an M1 chart, so the historical modes silently skipped the symbol the operator was looking at |
...
### F-059 (S1, process) — A 'git checkout --' discarded an uncommitted product change, and the commit message described the change that had just been lost

- **Status:** DONE  |  **Category:** tests  |  **Host:** node3  |  **Commit:** 3b0b072
- **Location:** `AUDIT/ledger.md, commit d165037`
- **Discovered by:** a grep for the identifier after the commit
- **Evidence (before):**

  > Committing the F-054 work needed a temporary edit reverted - HistoricalLookbackDays was set to 2900 for an evaluation run - so I ran 'git checkout -- FXNews.mq5'. The same file also held the real, uncommitted product change to the skip message, which was discarded with it and never committed. The commit that followed (d165037) states 'The product's skip message now says which case it is: for the chart symbol it names the M1-chart requirement'. It did not: 'grep -c chart_symbol FXNews.mq5' returned 0 and the committed text was the previous wording.

- **Fix:** The product change was re-derived and committed, and the claim in d165037 is corrected in this entry rather than by rewriting that commit, which is already pushed. The process rule this yields is recorded: a temporary edit and a real change must not share a file when 'git checkout --' is the revert mechanism, because it takes both and reports nothing.
- **Evidence (after):** 'grep -c "symbols[s] == _Symbol" FXNews.mq5' returns 1, the two messages were verified in a live run on an M5 chart (both fire, and neither refers to the removed ChartOpen attempt), and the proactive warning was verified to print before the run rather than after it. Gates: build 0/0, census 0, contracts 0/9, selftest 225/0.
- **Notes:** Found only because a later grep for the identifier came back empty. Nothing in the gate could have caught it - the build, census, contracts and self-test were all green both before and after, because the lost change was inside a string literal in a rarely-taken branch, and a commit message is not checked against the diff by anything. The general shape is the one this audit keeps finding: a claim of done-ness with no mechanical link to the thing claimed. Two other slips of the same family were already recorded - the 3.1 commit message claiming a self-test result before it was captured, and the wiki claiming 86 inputs from a stale local main - and both were also found by re-derivation rather than by a gate.
...
### F-054 (S3, reporting) — The chart symbol's M1 history is only built on an M1 chart, so the historical modes silently skipped the symbol the operator was looking at

- **Status:** DONE  |  **Category:** docs  |  **Host:** node3  |  **Commit:** d165037
...
- **Fix:** Cause established by measurement: the terminal builds the M1 series for whichever symbol the chart is on ONLY when the chart timeframe is M1; on any other timeframe CopyRates(chart_symbol, PERIOD_M1, range) returns 4401 while every other symbol downloads on demand. Fixed in two places: the gate runs the historical modes on an M1 chart, and FXNEWS_CHART_SYMBOL points the chart outside the evaluation list for windows longer than the chart's own load. The product's skip message now distinguishes the chart symbol from any other and names the actual remedy. PRODUCT SIDE, previously claimed in d165037 but NOT actually committed: the skip message now distinguishes the chart symbol from any other and names the measured remedy, and OnInit warns before a historical run when the chart symbol is in SymbolsToScan and the chart is not M1.
- **Evidence (after):** Chart=EURUSD on M5: 'Symbols 1/2', EURUSD skipped. Same run on M1: 'Symbols 2/2', EURUSD 92 005 bars and GBPUSD 91 891. For a 2900-day window an M1 chart is not enough - the chart symbol's series holds only what the chart loaded - so the 2900-day run with the chart on EURGBP (outside the list) gives 'Symbols 7/7', 20 665 917 M1 bars. Gates: build 0/0; census 0; contracts 0/9; selftest 225/0.
- **Notes:** CORRECTS THIS ENTRY'S EARLIER CONCLUSION. It was first closed with 'root cause not established' and a message-only fix; the cause was then found and it is a real defect, not a message problem. Ruled out by measurement before the cause was found: the .hcc files exist and are current; SymbolSelect changes nothing; a 420 s wait changes nothing; the positional CopyRates form fails too; and a separate script successfully requested 408 000 bars for all 7 symbols without helping the chart symbol. It is not EURUSD either - moving the chart to GBPUSD made GBPUSD the skipped one and loaded EURUSD. WHY IT MATTERED: the symbol an operator is most likely to evaluate is the one on their chart, and it was silently dropped from every historical report. 'Symbols 1/2' reads as a data problem. A hypothesis I had was measured and REJECTED on the way: adding SymbolSelect before the request, which was reverted rather than kept once it changed nothing. tools/mql5/FXNewsPrimeHistory.mq5 is kept as a utility with a header saying it does not fix this. DISPROVED, so it is not re-filed: opening a temporary M1 chart with ChartOpen(symbol, PERIOD_M1), polling 20 s and retrying does NOT produce the series - the terminal builds it only for the chart it STARTS on, and the helper chart printed its own failure message while EURUSD still returned 4401. The code was removed rather than kept behind the fallback, and the skip message was corrected so it no longer refers to a temporary chart that no longer exists. This is why the product fix is a proactive warning plus an accurate message and not a functional repair: the limitation is in the terminal, and the remedies are the chart timeframe at start (the gate now does this) or a chart symbol outside the evaluation list (FXNEWS_CHART_SYMBOL).
