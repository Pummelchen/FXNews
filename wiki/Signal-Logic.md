...
Signal messages are shown only at `75%` or higher. The visible message list refreshes at most every 10 seconds, sorts by displayed percentage descending, and suppresses repeated `symbol + timeframe + direction` rows when only the percentage changes.
